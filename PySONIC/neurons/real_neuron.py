...
# @Last Modified time: 2024-02-28 15:58:16
...
class RealisticNeuron(PointNeuron):
    ...
    def alpham_SKv31(cls,Vm):
        variables = tf.gating_from_PROCEDURES(Vm=Vm, list_mod=cls.mod_files[2], mod_name='SKv31')
    ...
    def alpham_CaHVA(cls,Vm):
        variables = tf.gating_from_PROCEDURES(Vm=Vm, list_mod=cls.mod_files[4], mod_name='CaHVA')
        return variables['m'+'alpha']
    ...
    def alpham_KdShu2007(cls,Vm):
        variables = tf.gating_from_PROCEDURES(Vm=Vm, list_mod=cls.mod_files[5], mod_name='KdShu2007')
        return variables['m'+'alpha']
    ...
    def alpham_Im(cls,Vm):
        variables = tf.gating_from_PROCEDURES(Vm=Vm, list_mod=cls.mod_files[7], mod_name='Im')
        return variables['m'+'alpha']
    ...
    def alpham_Ca(cls,Vm):
        variables = tf.gating_from_PROCEDURES(Vm=Vm, list_mod=cls.mod_files[9], mod_name='Ca')
        return variables['m'+'alpha']
    ...
    def alpham_CaLVAst(cls,Vm):
        variables = tf.gating_from_PROCEDURES(Vm=Vm, list_mod=cls.mod_files[10], mod_name='CaLVAst')
        return variables['m'+'alpha']
    ...
    def alpham_NaTat(cls,Vm):
        variables = tf.gating_from_PROCEDURES(Vm=Vm, list_mod=cls.mod_files[11], mod_name='NaTat')
        return variables['m'+'alpha']
    ...
    def alpham_KPst(cls,Vm):
        variables = tf.gating_from_PROCEDURES(Vm=Vm, list_mod=cls.mod_files[12], mod_name='KPst')
        return variables['m'+'alpha']
    ...
    def alpham_NaTs2t(cls,Vm):
        variables = tf.gating_from_PROCEDURES(Vm=Vm, list_mod=cls.mod_files[13], mod_name='NaTs2t')
        return variables['m'+'alpha']
    ...
    def alpham_NapEt2(cls,Vm):
        variables = tf.gating_from_PROCEDURES(Vm=Vm, list_mod=cls.mod_files[18], mod_name='NapEt2')
        return variables['m'+'alpha']
    ...
    def i_SKv31(cls,m_SKv31,Vm):
        ...
        variables = tf.currents_from_BREAKPOINT(list_mod=cls.mod_files[2], mod_name='SKv31', Vm=Vm, x_dict = x_dict, g_dict = cls.g_dict, location = "somatic")
    ...
    def i_CaHVA(cls,m_CaHVA,h_CaHVA,Vm):
        ...
        variables = tf.currents_from_BREAKPOINT(list_mod=cls.mod_files[4], mod_name='CaHVA', Vm=Vm, x_dict = x_dict, g_dict = cls.g_dict, location = "somatic")
        currents = [e for e in variables.keys() if (e.startswith('i') or e.startswith('I'))]
        print(currents)
        if currents:
            return variables[currents[0]]
    ...
    def i_KdShu2007(cls,m_KdShu2007,h_KdShu2007,Vm):
        ...
        variables = tf.currents_from_BREAKPOINT(list_mod=cls.mod_files[5], mod_name='KdShu2007', Vm=Vm, x_dict = x_dict, g_dict = cls.g_dict, location = "somatic")
        currents = [e for e in variables.keys() if (e.startswith('i') or e.startswith('I'))]
        print(currents)
        if currents:
            return variables[currents[0]]
    ...
    def i_Im(cls,m_Im,Vm):
        ...
        variables = tf.currents_from_BREAKPOINT(list_mod=cls.mod_files[7], mod_name='Im', Vm=Vm, x_dict = x_dict, g_dict = cls.g_dict, location = "somatic")
        currents = [e for e in variables.keys() if (e.startswith('i') or e.startswith('I'))]
        print(currents)
        if currents:
            return variables[currents[0]]
    ...
    def i_Ca(cls,m_Ca,h_Ca,Vm):
        ...
        variables = tf.currents_from_BREAKPOINT(list_mod=cls.mod_files[9], mod_name='Ca', Vm=Vm, x_dict = x_dict, g_dict = cls.g_dict, location = "somatic")
        currents = [e for e in variables.keys() if (e.startswith('i') or e.startswith('I'))]
        print(currents)
        if currents:
            return variables[currents[0]]
    ...
    def i_CaLVAst(cls,m_CaLVAst,h_CaLVAst,Vm):
        ...
        variables = tf.currents_from_BREAKPOINT(list_mod=cls.mod_files[10], mod_name='CaLVAst', Vm=Vm, x_dict = x_dict, g_dict = cls.g_dict, location = "somatic")
        currents = [e for e in variables.keys() if (e.startswith('i') or e.startswith('I'))]
        print(currents)
        if currents:
            return variables[currents[0]]
    ...
    def i_NaTat(cls,m_NaTat,h_NaTat,Vm):
        ...
        variables = tf.currents_from_BREAKPOINT(list_mod=cls.mod_files[11], mod_name='NaTat', Vm=Vm, x_dict = x_dict, g_dict = cls.g_dict, location = "somatic")
        currents = [e for e in variables.keys() if (e.startswith('i') or e.startswith('I'))]
        print(currents)
        if currents:
            return variables[currents[0]]
    ...
    def i_KPst(cls,m_KPst,h_KPst,Vm):
        ...
        variables = tf.currents_from_BREAKPOINT(list_mod=cls.mod_files[12], mod_name='KPst', Vm=Vm, x_dict = x_dict, g_dict = cls.g_dict, location = "somatic")
        currents = [e for e in variables.keys() if (e.startswith('i') or e.startswith('I'))]
        print(currents)
        if currents:
            return variables[currents[0]]
    ...
    def i_NaTs2t(cls,m_NaTs2t,h_NaTs2t,Vm):
        ...
        variables = tf.currents_from_BREAKPOINT(list_mod=cls.mod_files[13], mod_name='NaTs2t', Vm=Vm, x_dict = x_dict, g_dict = cls.g_dict, location = "somatic")
        currents = [e for e in variables.keys() if (e.startswith('i') or e.startswith('I'))]
        print(currents)
        if currents:
            return variables[currents[0]]
    ...
    def i_NapEt2(cls,m_NapEt2,h_NapEt2,Vm):
        ...
        variables = tf.currents_from_BREAKPOINT(list_mod=cls.mod_files[18], mod_name='NapEt2', Vm=Vm, x_dict = x_dict, g_dict = cls.g_dict, location = "somatic")
        currents = [e for e in variables.keys() if (e.startswith('i') or e.startswith('I'))]
        print(currents)
        if currents:
            return variables[currents[0]]
