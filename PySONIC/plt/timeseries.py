...
class GroupedTimeSeries(TimeSeriesPlot):
    ...
    def render(self, fs=10, lw=2, labels=None, colors=None, lines=None, patches='one', save=False,
        ...
        for output in self.outputs:
            ...
            ABERRA = 1 if ('a=' in str(fiber) and 'fs=' in str(fiber)) else 0
                ...
                for (curr, curr_lambda),(cond, cond_lambda) in zip(fiber.pneuron.currents().items(),fiber.pneuron.conductances().items()): #iterate over all the currents listed in the specific pneuron
                    ...
                    try:
                        ...
                        if nargs == 1: #this is currently only in the case of the passive mechanism
                            data[curr] = curr_lambda(data['Vm'], None, None)
                            curr_add.append(curr)
                            data['i_net'] += data[curr]
                            continue
                        x = {f'm_{suffix}': data[("m", suffix)]} #init of x, which always contains m and h if it exists
                        if nargs > 3:
                            if nargs != 4:
                                TypeError('More than 4 arguments for a cls.i method?')
                            x[f'h_{suffix}'] = data[("h", suffix)] #add h if it is needed as an argument
            ...
            directory = r'C:\Users\jgazquez\OneDrive - UGent\PhD\Figures\self_made\run_realistic_astim output\try 25\\'
            ...
            data.to_csv(f'{directorycsv}\\{filename}.csv')
